...
@pytest.mark.parametrize(
    "out, license_info, serial, pin, previous_pin, path",
    [
...
def test_success(monkeypatch, license_manager, out, license_info, serial, pin, previous_pin, path):
    monkeypatch.setattr(subprocess, "run", lambda args, **kwargs: success_result(args, out))
    assert license_manager.activate(
...
@pytest.mark.parametrize(
    "out, return_code, license_info, serial, pin, previous_pin, path",
    [
...
def test_fail(monkeypatch, license_manager, return_code, out, license_info, serial, pin, previous_pin, path):
