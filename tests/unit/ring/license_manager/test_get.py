...
def test_success(monkeypatch, license_manager, name, expected):
    ...
    assert hashlib.md5(license_manager.get(name=name)).hexdigest() == expected
...
def test_fail(monkeypatch, license_manager, out, return_code, path):
    ...
    with pytest.raises(subprocess.CalledProcessError):
        license_manager.get(name="111111111111111-1111111111", path=path)
