...
@pytest.mark.parametrize("license, out", [(b"test_license", "Лицензия успешно добавлена.\n"), ], ids=["put_license"])
def test_success(monkeypatch, license_manager, license, out):
    monkeypatch.setattr(subprocess, "run", lambda args, **kwargs: success_result(args, out))
    assert license_manager.put(license=license)
...
def test_fail(monkeypatch, license_manager, out, return_code, path):
    ...
    with pytest.raises(subprocess.CalledProcessError):
        license_manager.put(license=b"test_license", path=path)
